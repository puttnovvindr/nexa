...
export default function ManualImportForm({ 
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    handleAction(
      saveManualEmployee(submissionData),
        ...
        <div className="h-[240px] overflow-y-auto px-1 pr-4 custom-scroll w-full scroll-smooth">
          <div className={cn("space-y-6 animate-in fade-in duration-300", activeStep !== 0 && "hidden")}>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormSelect label="Gender" name="gender" placeholder="Select gender" defaultValue={formData.gender} onChange={(val) => setFormData(p => ({...p, gender: String(val)}))} options={[{label: "Male", value: "MALE"}, {label: "Female", value: "FEMALE"}]} />
          ...
          <div className={cn("space-y-6 animate-in fade-in duration-300", activeStep !== 1 && "hidden")}>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormSelect label="Department" name="orgUnitId" placeholder="Select department" options={unitOptions} defaultValue={formData.orgUnitId} error={touched ? (errors.orgUnitId ?? undefined) : undefined} onChange={(val) => setFormData(p => ({...p, orgUnitId: String(val), jobId: ""}))} required />
                <FormSelect label="Job Title" name="jobId" placeholder="Select job title" defaultValue={formData.jobId} error={touched ? (errors.jobId ?? undefined) : undefined} options={filteredJobs} onChange={(val) => setFormData(p => ({...p, jobId: String(val)}))} required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <FormSelect label="Job Level" name="jobLevelId" placeholder="Select level" defaultValue={formData.jobLevelId} error={touched ? (errors.jobLevelId ?? undefined) : undefined} options={jobLevels.map(l => ({label: l.levelName, value: l.id}))} onChange={(val) => setFormData(p => ({...p, jobLevelId: String(val)}))} required />
                <FormSelect label="Employment Type" name="employmentTypeId" placeholder="Select type" defaultValue={formData.employmentTypeId} error={touched ? (errors.employmentTypeId ?? undefined) : undefined} options={employmentTypes.map(t => ({label: t.name, value: t.id}))} onChange={(val) => setFormData(p => ({...p, employmentTypeId: String(val)}))} required />
              </div>
              <FormSelect label="Work Schedule" name="workScheduleId" placeholder="Select schedule" defaultValue={formData.workScheduleId} onChange={(val) => setFormData(p => ({...p, workScheduleId: String(val)}))} options={workSchedules.map((s) => ({ label: s.shiftName, value: s.id }))} />
            </div>
          </div>
          ...
          <div className={cn("space-y-6 animate-in fade-in duration-300", activeStep !== 3 && "hidden")}>
            <div className="space-y-4">
              ...
              <FormSelect label="PTKP Status" name="ptkpStatus" placeholder="Select status" defaultValue={formData.ptkpStatus} onChange={(val) => setFormData(p => ({...p, ptkpStatus: String(val)}))} 
                options={[
